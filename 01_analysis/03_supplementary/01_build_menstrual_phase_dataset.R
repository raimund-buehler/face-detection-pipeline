...
if ("Male or female" %in% names(df)) {
    df <- df %>% rename(Gender = "Male or female")
}
if ("Codeeingabe: Code" %in% names(df)) {
    df <- df %>% rename(ID = "Codeeingabe: Code")
}
if ("Datum Zyklus: (Tag) ... " %in% names(df)) {
    df <- df %>% rename(cycle_day = "Datum Zyklus: (Tag) ... ")
}
if ("Datum Zyklus: (Monat) ... 2022" %in% names(df)) {
    df <- df %>% rename(cycle_month = "Datum Zyklus: (Monat) ... 2022")
}
...
# Define a function to categorize the menstrual phase using
# participant-specific cycle length:
# - Follicular: day <= L - 15
# - Luteal: day >= L - 14
get_menstrual_phase <- function(cycle_day, cycle_length) {
    if (
        is.na(cycle_day) || !is.numeric(cycle_day) ||
        is.na(cycle_length) || !is.numeric(cycle_length)
    ) {
        return(NA)
    }

    if (cycle_day <= (cycle_length - 15)) {
        return("Follicular")
    } else if (cycle_day >= (cycle_length - 14)) {
        return("Luteal")
    } else {
        return(NA)
    }
...
menst <- menst %>%
    mutate(
        phase_1 = mapply(get_menstrual_phase, effective_cycle_day_1, Zyklustage_num),
        phase_2 = mapply(get_menstrual_phase, effective_cycle_day_2, Zyklustage_num),
        phase_3 = mapply(get_menstrual_phase, effective_cycle_day_3, Zyklustage_num),
        phase_4 = mapply(get_menstrual_phase, effective_cycle_day_4, Zyklustage_num)
    )

write_csv(menst, here("00_data", "derived", "assembly", "menst_all.csv"))
